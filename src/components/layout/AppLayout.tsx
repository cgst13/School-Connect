...
export function AppLayout({
  ...
  useEffect(() => {
    if (displayTitle) {
      document.title = `${displayTitle} | School Connect`
    }
  }, [displayTitle])
    ...
    <div className="h-dvh max-h-dvh relative bg-gradient-to-br from-[#F4EFFC] via-[#EBF3FE] to-[#FFF1F6] text-[#2D2638] flex flex-col w-full overflow-hidden font-sans">
      ...
      <header className="relative z-40 shrink-0 bg-transparent px-4 sm:px-8 py-3.5 flex items-center justify-between no-print">
        <div className="flex items-center gap-4">
          ...
          {/* Top Header Official Logo & User Greeting */}
          <div className="flex items-center gap-4 min-w-0">
            ...
            <div className="hidden xs:block min-w-0 border-l border-slate-200/80 pl-4">
              ...
              <p className="text-xs text-[#7A7289] font-medium hidden sm:block truncate">
                Welcome to TERMCAT System
              </p>
            </div>
          </div>
        </div>

        {/* Right Header Actions & Search Pill */}
      ...
      <div className="flex-1 flex min-w-0 min-h-0 overflow-hidden px-4 sm:px-6 pb-4 gap-5">
        ...
        {hasSidebar && (
          ...
          >
            {/* 3D Profile Frame */}
            {!isCollapsed ? (
              <div className="text-center pb-4 mb-2 border-b border-purple-200/50 relative">
                <button
                  onClick={toggleCollapse}
                  className="absolute right-0 top-0 p-1.5 rounded-xl bg-white/80 text-[#7A7289] hover:text-[#8B72F4] transition-all cursor-pointer border border-white"
              ...
              <div className="text-center pb-3 mb-2 border-b border-purple-200/50 flex flex-col items-center gap-2">
                <button
                  onClick={toggleCollapse}
                  className="p-1.5 rounded-xl bg-white/80 text-[#7A7289] hover:text-[#8B72F4] transition-all cursor-pointer border border-white"
        ...
        {mobileOpen && (
          <div className="fixed inset-0 z-50 lg:hidden flex animate-fade-in">
            ...
            <div className="relative w-64 max-w-[80vw] h-full bg-[#EFE6FA] border-r border-white/80 flex flex-col z-10 p-4 space-y-4 shadow-xl">
              <div className="flex items-center justify-between pb-3 border-b border-purple-200/50">
                <span className="text-sm font-black text-[#2D2638]">{displayTitle}</span>
                <button onClick={() => setMobileOpen(false)} className="p-2 rounded-2xl bg-white shadow-xs text-[#7A7289]">
                  <X size={18} />
                </button>
              </div>
              ...
              <nav className="flex-1 space-y-4 overflow-y-auto">
                {navGroups.map(group => (
                  <div key={group.title} className="space-y-1.5">
                    <div className="text-[10px] font-bold uppercase tracking-wider text-[#A39BAF] mb-1">
                      {group.title}
                    </div>
                    {group.items.map(item => (
                      <Link
                        ...
                        className="flex items-center gap-3 py-3 px-3.5 rounded-2xl text-xs font-bold text-[#7A7289] hover:bg-white hover:text-[#8B72F4]"
                      >
                        <span>{item.icon}</span>
              ...
              <div className="pt-3 border-t border-purple-200/50 mt-auto shrink-0">
                <Link
                  to="/portal"
                  onClick={() => setMobileOpen(false)}
                  className="flex items-center gap-2.5 py-3 px-3.5 rounded-2xl text-xs font-bold text-[#8B72F4] bg-white shadow-xs"
                >
                  <ArrowLeft size={16} />
